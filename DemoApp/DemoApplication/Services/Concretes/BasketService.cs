...
namespace DemoApplication.Services.Concretes
{
    public class BasketService : IBasketService
    {
        ...
        public async Task<List<ProductCookieViewModel>> AddBasketProductAsync(Book book)
        {
            ...
            {
                book = _dataContext.Books.Include(b => b.BookImages).FirstOrDefault(b => b.Id == book.Id);
                ...
                if (productCookieViewModel is null)
                {
                    productsCookieViewModel
                        !.Add(new ProductCookieViewModel(book.Id, book.Title,
                        book.BookImages.Take(1).FirstOrDefault()! != null ? _fileService.GetFileUrl(book.BookImages!.Take(1)!.FirstOrDefault()!.ImageNameInFileSystem!, UploadDirectory.Book) : String.Empty,
                        1, book.Price, book.Price));
                }
                else
                {
